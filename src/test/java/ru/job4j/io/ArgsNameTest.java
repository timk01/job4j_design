...
class ArgsNameTest {
    ...
    @Test
    void whenKeyNotExist2() {
        ArgsName jvm = ArgsName.of(new String[] {"-encoding=UTF-8"});
        assertThatThrownBy(() -> jvm.get("encoder")).isInstanceOf(IllegalArgumentException.class)
                .hasMessageMatching("^.+")
                .hasMessageContaining("This key: 'encoder' is missing");
    }
    ...
    void whenThereNoEqualSignThenExceptionThrown2() {
        assertThatThrownBy(() -> ArgsName.of(new String[]{"-Xmx512", "-request=?msg=Hello="}))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageMatching("^.+")
