...
public class BinarySearchTree<T extends Comparable<T>> {
    ...
    /**
     * 1 - 2 (левый 1 уровня) - 3 - 4 (рут) - 5 - 6 - 7
     * дошел до конца (левый, 1). если увидел что там нет потомков, провалился в ИФ, но сразу выскочил
     * вернулся обратно
     * обратно в узле-родителе (2): зашел в правый узел (3), он - нуль, вернулся к вызову из 2,
     * обнулил у узла (2) его кей + потомков (сам узел пока достижим, но потомки и кей - занулены)
     * веернулся к узлу (4)
     * принмиаюсь за правую сторону: 5-6-7, от 6 (правого) берем левый и повторяем процесс
     * т.е. там обнулятся кей + потомки у 6 (пока достижм),
     * потом вернемся к 4, обнулим кей, 2 (левый - уже обнулен, полностью), 6 (правый - тоже)
     * вернемся к вызывающему
     * <p>
     * в концее - т.к. се от корня - нули - по идее надо обнулить корень
     */
